...
for($x=0;$x<count($id_soalan);$x++)
{
	...
	$query_answer = "select ID_PENILAIAN_PESERTA_JAWAPAN from  usr_isac.prs_penilaian_peserta_jawapan where id_soalan = '$id_soalan1' and id_peserta = '$usr' and id_permohonan='$app'";
	$result_answer = mysql_query($query_answer) or die('Error, query failed');

	if(mysql_fetch_array($result_answer)>0)
	{
	$answered = "<a href='soalan_perincian1.php?page=".($x+1)."&submit=true&usr=$usr&app=$app'><span style='color:blue;font-size:13px;'><u>Telah Dijawab</u></span></a>";
	$icon = "green.png";
	}
	
	else
	{
	$answered = "<a href='soalan_perincian1.php?page=".($x+1)."&submit=true&usr=$usr&app=$app'><span style='color:red;font-size:13px;'><u>Belum Dijawab</u></span></a>";
	$icon = "red.png";
	}
...
<input style="margin-right:3px" id="submit" name="submit" type="submit" value="Hantar" class="inputButton" onClick="confirmation();saveMasa2(<?php echo $app;?>);">
</div>
</form>
