...
if(count($checkRs)==0)
{
	...
	$kodIAC = $resultKodIAC[0][0];
        ...
        //added 20120612
        $date1=mktime(date("H")+8,date("i"),date("s"),date("m"),date("d"),date("Y"));
        $currentYear=date("Y",$date1);

        $checkYear= "select max(tahun) from usr_isac.pro_sijil where kod_iac = '".$kodIAC."'";
        $resultYear = $myQuery->query($checkYear,'SELECT');
        $sijilYear = $resultYear[0][0];
	if($currentYear != $sijilYear)	
		{
		$sqlSijil = "insert into usr_isac.pro_sijil(no_sijil,kod_iac,id_peserta,id_permohonan,tahun,status_push_pull) values ('1','".$kodIAC."','".$usr."','".$app."',".$currentYear.",'I')";
		$resultSijil = $myQuery->query($sqlSijil,'RUN');
		
		}
		else
		{
		$checkNoSijil = "select max(no_sijil)+1 from usr_isac.pro_sijil where kod_iac = '".$kodIAC."' and tahun='".$currentYear."'";
		$resultNoSijil = $myQuery->query($checkNoSijil,'SELECT');
		$noSijil = $resultNoSijil[0][0];
				
		$sqlSijil = "insert into usr_isac.pro_sijil(no_sijil,kod_iac,id_peserta,id_permohonan,tahun,status_push_pull) values ('".$noSijil."','".$kodIAC."','".$usr."','".$app."',".$currentYear.",'I')";
		$resultSijil = $myQuery->query($sqlSijil,'RUN');
		}//eof check year
	}//eof lulus
