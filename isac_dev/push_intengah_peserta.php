<?php
include('D:/xampp/htdocs/isac/class/db_mysql.php');
include('D:/xampp/htdocs/isac/class/sql_mysql.php');
include ('db.php');
...
$query = "select b.id_penyelaras, a.id_peserta, a.nama_peserta, a.no_kad_pengenalan, a.no_kad_pengenalan_lain, a.status_push_pull,
...
a.id_peserta = b.id_peserta and b.id_sesi = c.id_sesi and a.id_peserta = d.id_peserta and c.kod_iac = '$kod_iac' and
(a.status_push_pull = 'I' or a.status_push_pull = 'U')";

$result = $myQuery->query($query ,'SELECT','NAME');

$query2 = "select * from pro_sesi where kod_iac = '$kod_iac' and
(status_push_pull = 'I' or status_push_pull = 'U')";
...
for($c=0;$c<count($result2);$c++){
	...
	if($result2[$c][STATUS_PUSH_PULL] == 'I'){
		 ...
		 '".$result2[$c][KOD_IAC]."','".$result2[$c][KOD_STATUS]."','".$result2[$s][ID_PENGHANTARAN]."','".$result2[$c][KOD_JENIS_SESI]."',
	...
	//ni ko tolong qus...byk sgt field die :)
		$query = mysql_query("update pro_sesi set ");
...
for($s=0;$s<count($result);$s++){
	...
	if($result[$s][STATUS_PUSH_PULL] == 'I'){
		 ...
		 ('".$result[$s][ID_PENYELARAS]."','".$result[$s][ID_PESERTA]."','".$result[$s][NAMA_PESERTA]."','".$result[$s][NO_KAD_PENGENALAN]."',
		 ...
		 $query3 = mysql_query("insert into pro_perkhidmatan (id_perkhidmatan,kod_klasifikasi_perkhidmatan,id_peserta,status_push_pull) values
		 ('".$result[$s][ID_PERKHIDMATAN]."','".$result[$s][KOD_KLASIFIKASI_PERKHIDMATAN]."','".$result[$s][ID_PESERTA]."','N')");
	...
	elseif($result[$s][STATUS_PUSH_PULL] == 'U'){
		...
		$query6 = mysql_query("update pro_perkhidmatan set kod_klasifikasi_perkhidmatan = '".$result[$s][KOD_KLASIFIKASI_PERKHIDMATAN]."', id_peserta = '".$result[$s][ID_PESERTA]."', status_push_pull = 'N'");
