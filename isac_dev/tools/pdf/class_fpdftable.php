...
function Draw_Header_( $next_line_height = 0 ){
	...
	for($i=0;$i<$this->tb_columns;$i++)
	{
		...
		$ismal_tambah = 1.5;
		$h = max($h, $this->tb_header_type[$i]['LN_SIZE'] * $this->tb_header_type[$i]['CELL_LINES'] + $ismal_tambah);
...
function Draw_Data2($data, $header = true){
...
function MultiCellTable2($w, $h, $txt, $border=0, $align='J', $valign='T', $fill=0, $vh=0)
{
	...
	*/
	...
	$this->MultiCell2($w,$h,$txt,$b2,$align,$fill);
...
function Footer() {
	...
	$section = "";
	if (isset($GLOBALS['report_section']) && trim($GLOBALS['report_section'] != "")) {
		$section = $GLOBALS['report_section'];
		$this->Cell(0,10,'S' . $section . "/" . $this->PageNo(),0,0,'C');
	} 
	
	if (isset($GLOBALS['update_on']) && trim($GLOBALS['update_on'] != "")){
		$update_on = $GLOBALS['update_on'];		
		if ($GLOBALS['form_orientation'] == "P"){
			$this->SetXY(190,-18);
		} else {
			$this->SetXY(280,-18);
		}	
		$this->Cell(10,10,"Kemaskini : ".$update_on,0,0,'R');
	}

	if (isset($GLOBALS['print_on']) && trim($GLOBALS['print_on'] != "")){
		$print_on = date("d M Y g:h a");
		if ($GLOBALS['form_orientation'] == "P"){
			$this->SetXY(185,-15);
		} else {
			$this->SetXY(275,-15);
		}		
		$this->Cell(10,10,"Print on : ".$print_on,0,0,'R');
	}

	if(function_exists('DrawFooter')){
		DrawFooter($GLOBALS['pdf']);	
	}
}
