...
function display() {
  ...
  if (isWriteable) {
    ...
    out += "<a href='#' onclick='loadCode(); return false;' accesskey='l' style='display:none'>"+trans("Load")+"</a>";		//edited by fais (display:none)
    out += "<a href='#' id=\"save_spreadsheet_data\" onclick='eval(saveMethod); return false;' accesskey='s' style='display:none'>"+trans("Save")+"</a>";		//edited by fais (display:none)
  }
  if (top.window!=window && closeMethod!="") out += "<a href='#' onclick='if (confirm(\"Really close ?\")) eval(closeMethod); return false;' accesskey='q'>"+trans("Close")+"</a> - ";
  ...
  out += "<input type='text' title='"+trans("Style")+"' value='' id='styling' style='width:33%; display:none' disabled onmouseover='previewValue();' onkeyup='previewValue();'> ";
  out += "</td><td nowrap>";
  if (isWriteable) {
    out += "&nbsp; <input type='button' value='"+trans("Save")+"' id='save' onclick='saveCell();' disabled style=\"display:none;\">";
    out += "<input type='button' value='"+trans("X")+"' id='cancel' onclick='cancelCell();' disabled style=\"display:none;\">";
  } else out += "&nbsp; ";
/*  if (view=="values") {
    if (isWriteable) {
  ...
  out += "<a href='#' onclick='view=\"styles\"; display(); return false;' accesskey='3'></a>";*/	//commented fais 20091123
  ...
  //out += "<a href='#' onclick='save(\"csv\"); return false;'>"+trans("Export to CSV")+"</a> - ";	//commented fais 20091123
  //out += "<a href='#' onclick='save(\"tsv\"); return false;'>"+trans("Export to TSV")+"</a>";		//commented fais 20091123
  out += "</div>";
  ...
  if (number.length > 3) {
    ...
    for (i=0; i<Math.floor(number.length/3); i++) {
      if (mod==0 && i==0) {
        ...
        //output += ","+number.substring(mod+3*i,mod+3*i+3);	//commented by fais 20091117
		output += number.substring(mod+3*i,mod+3*i+3);
      }
    }
  } else output += number;
