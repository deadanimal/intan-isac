...
*/
	...
	$active = "select TEMPOH_KEBENARAN_PERMOHONAN_PESERTA_BLACKLIST from usr_isac.pro_kawalan_sistem where id_kawalan_sistem=1";
	...
	$chkStatus = "SELECT b.kod_status_kehadiran,d.kod_status
						...
						and curdate() > a.tarikh_sesi and (curdate() <= (adddate(a.tarikh_sesi,'$active_blacklist')))
						and (c.no_kad_pengenalan='".$_POST['publicID']."' or c.no_kad_pengenalan_lain='".$_POST['publicID']."')";
	$chkStatusRs = $myQuery->query($chkStatus,'SELECT');
	//if status =02 (tidak hadir penilaian)
	if($chkStatusRs[0][0]=='02' && $chkStatusRs[0][1]!='01')
	{	
	...
	{	
		...
		$daftar="select a.id_peserta
				...
				b.kod_kategori_peserta='02' and a.kod_status_permohonan='01' 
...
else if($_POST['login_penilaian'])
{
	...
	$nilai="select a.id_peserta,a.id_sesi
				...
				and c.tarikh_sesi = curdate()";
	$nilaiRs=$myQuery->query($nilai,'SELECT');
	$id_sesi=$nilaiRs[0][1];
	
	//check kod status kelulusan
	$nilai2="select d.kod_status_kelulusan
				from usr_isac.prs_permohonan a, 
				usr_isac.pro_peserta b,
				usr_isac.pro_sesi c,
...
else if ($nilaiRs)
{
	...
	$_SESSION['idPenilaian']=$_POST['idPenilaian'];
		...
		$sql_check = "select a.id_penilaian_peserta
							from 
							usr_isac.prs_penilaian_peserta a,
							usr_isac.pro_peserta b
							where 
							a.id_peserta = b.id_peserta
							and
							(b.no_kad_pengenalan='".$_POST['publicID']."' or b.no_kad_pengenalan_lain='".$_POST['publicID']."')";
		$checkRs = $myQuery->query($sql_check,'SELECT');
		//$id_peserta = $checkRs[0][1];
		
		if(count($checkRs)==0)
		{
		$sql = "insert into usr_isac.prs_penilaian_peserta (kod_status_kehadiran,id_penilaian,id_peserta) values ('01','".$id_nilai."','".$id_peserta."')";
		$result = $myQuery->query($sql,'RUN');
		
		//update status permohonan, kod_status = 02(telah menduduki penilaian,permohonan=inactive)
		$sql3 = "update usr_isac.prs_permohonan set kod_status_permohonan = '02',kod_status_kehadiran='01' where id_peserta='".$id_peserta."'";
