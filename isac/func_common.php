...
function notification($notificationName)
{
	...
	if(is_array($tempField)&&is_array($tempValue)&&$emailFlag)
	{
		$insert = "insert into FLC_NOTIFICATION_ITEM
					(NOTIFICATION_ITEM_ID,NOTIFICATION_ID,REQUEST_USER_ID,TIME_STAMP,NOTIFICATION_ITEM_STATUS,".implode(',',$tempField).")
					values
		...
		case 'csv':
			...
			for($x=0;$x<$rowsCount;$x++)
			{
				...
				if(trim($rows[$x])!='')
				{
					...
					{
						$_SESSION['loading']['content'][$x][$y] = str_replace("\n",'',$columns[$y]);
					}//eof for
				}//eof if
			}//eof for
		break;
	...
	switch($type)
	{
		case 'csv':
			...
			for($x=0;$x<$tempInputCount;$x++)
			{
				...
				//get data, exclude header
				if($inputChunks[0]>=0&&$inputChunks[1]>=0)
					$loading_content[trim($inputChunks[1])][trim($inputChunks[0])]=$inputChunks[2];
			}//eof for
		break;
	}//eof switch
