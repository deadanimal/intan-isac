...
class TableGrid extends Table
{
	...
	private $addRowCount;			//set total number of rows that can be added
	...
	public function showTableGrid($input='')
	{
		...
		{
			//======== ROW TO APPEND =======
			//row that can be added
			if(!$this->addRowCount)
				$this->addRowCount=10;
			
			//loop on count of add row
			for($x=0;$x<$this->addRowCount;$x++)					
			{
		...
		if($this->addRowStatus)
		{
			...
			if($this->runningStatus)
			{
				...
				var addRowRunningNumber = <?php echo str_replace('.','',$numbering);?>;
				</script>
				<?php
			}//eof if
			...
			//replace " with \' for javascript (input)
			...
			$tempT->setAttribute('style','border:0px');		//set border 0 in css
			...
			if($_GET['menuID']==116 || $_GET['menuID']==115 ){
			echo 	'<input class="inputInput" size="6" type="text" readonly="true" id="total" name="total" />&nbsp;&nbsp;&nbsp';
			}
			...
			<input type="button" id="<?php echo $this->addRowId;?>" name="<?php echo $this->addRowId;?>" class="<?php echo $this->addRowClass;?>" value="<?php echo $this->addRowValue;?>" <?php if($this->addRowDisabled){?> disabled="disabled"<?php }?> style="margin-top:5px; <?php if($this->addRowDisabled){?> color:#666666;<?php }?>"
				...
				<?php 
					...
					if($this->addRowType=='ajax'){?>
						var a = document.getElementById('<?php echo $currentLayer;?>').getElementsByTagName('tr');
						var b = a[a.length-1].getElementsByTagName('td');
						var c = b[0].innerHTML;
						var d = parseInt(c.split('.'))+1;
						refNum=d;
						
						var i = document.getElementById('addRow'+<?php echo $randTableGrid;?>).getElementsByTagName('tr');
						var l=0;
	
						if(i.length>0)
						{
							var j = i[i.length-1].getElementsByTagName('td');
							
							if(j.length>1)
							{
								var k = j[0].innerHTML;
								l = parseInt(k.split('.'))+1;
							}
						}
						...
						appendNewRow('addRow<?php echo $randTableGrid;?>','tableRowSub<?php echo $randTableGrid;?>','<?php echo $tempAtt;?>','<?php echo $tempInput;?>',refNum);
					<?php }
					//else if js add row
	...
	//----- add row -------
	public function setAddRowCount($addRowCount)
	{
	...
	public function setDelRowValue($delRowValue='Add New Row')
	{
...
{	
	...
	var ajax = new Ajax.Updater({success: rowToAdd},url,{method: 'post', parameters: params, onFailure: getFailureReport});
}
...
<?php
	...
	{
		echo '<td '.$attribute.'>'.$output[$x].'</td>';		//display data by column
	}//eof or
}//eof if
