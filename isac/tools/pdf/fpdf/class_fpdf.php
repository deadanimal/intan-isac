...
function AutoCellFontChange($txt) {
	$currsign = substr(trim($txt), 0, 7);
	switch ($currsign) {
		case "<%!g!%>": // grey
...
{
	$txt = $this->AutoCellFontChange($txt);
	//Output a cell
	$k=$this->k;
	if($this->y+$h>$this->PageBreakTrigger && !$this->InFooter && $this->AcceptPageBreak())
	{
...
function MultiCell2($w,$h,$txt,$border=0,$align='J',$fill=0)
{
	...
	while($i<$nb)
	{
		...
		{		
			...
			$this->Cell2($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
			$i++;
			$sep=-1;
			$j=$i;
			$l=0;
		...
		if($l>$wmax)
		{
			...
			else
			{
				...
				$this->Cell2($w,$h,substr($s,$j,$sep-$j),$b,2,$align,$fill);
				$i=$sep+1;
			}
			$sep=-1;
			$j=$i;
...
/*******************************************************************************
*                                                                              *
*                              Fungsi tambahan, class extension                *
*                                                                              *
*******************************************************************************/
